...
1. Generate the wildcard certificate for the AKS ingress controller.
   ...
   ```bash
   openssl req -x509 -nodes -days 365 -newkey rsa:2048 -out nginx-internal-ingress-controller-tls.crt -keyout nginx-internal-ingress-controller-tls.key -subj "/CN=*.aks-ingress.${DOMAIN_NAME_AKS_BASELINE}/O=Contoso AKS Ingress"
   ```
...
1. Base64 encode the AKS ingress controller certificate.
   ...
   ```bash
   export AKS_INGRESS_CONTROLLER_CERTIFICATE_BASE64_AKS_BASELINE=$(cat nginx-internal-ingress-controller-tls.crt | base64 | tr -d '\n')
