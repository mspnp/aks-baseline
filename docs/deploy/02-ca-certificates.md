...
1. Generate the wildcard certificate for the AKS ingress controller.
   ...
   > :book: Contoso Bicycle procured a standard CA certificate to be used with the AKS ingress controller. This one is not EV, because it won't be user-facing. The app team decides to use a wildcard certificate of `*.aks-ingress.contoso.com` for the ingress controller.
