...
1. Obtain the Azure Bastion Public IP Address

   > Note: this public ip address is used to configured the cluster Authorized IP addresseses than can access the AKS API Server

   ```bash
      ```bash
   BASTIONHOST_PIP=$(az deployment group show -g rg-enterprise-networking-hubs-${LOCATION_AKS_BASELINE} -n hub-regionA --query properties.outputs.bastionHostPIP.value -o tsv)
   echo BASTIONHOST_PIP: $BASTIONHOST_PIP

   CLUSTER_AUTHORIZED_IP_RANGES=[\"${BASTIONHOST_PIP}\"]
   ```
