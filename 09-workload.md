...
1. Customize the host name of the Ingress resource to match your custom domain

   > An example file is provided that will be used later with the built-in kubectl customization tool, Kustomize. This lets you patch the default Ingress resource definition with your own custom values.

   ```bash
   sed "s/<domain-name>/${DOMAIN_NAME}/g" workload/aspnetapp-ingress-patch.yaml.example > workload/aspnetapp-ingress-patch.yaml
   ```
