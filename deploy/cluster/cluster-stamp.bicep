...
module monitoringAlerts 'modules/monitoring-alerts.bicep' = {
  ...
  params: {
    ...
    containerInsightsName: sci.name  
  }
}
...
resource mc 'Microsoft.ContainerService/managedClusters@2024-03-02-preview' = {
  ...
  dependsOn: [
    ...
    // Policies that we want in place before the cluster is deployed and pods are added.
    // Some policies are Azure resource provider policies while others are
    // Azure Policy for Kubernetes policies.
